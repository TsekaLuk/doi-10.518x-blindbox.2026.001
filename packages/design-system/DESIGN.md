...
Note: this file is the human/agent design reference. Do not import or copy the raw Quick Start CSS snippets directly into the app; they contain generic token names that can collide with Astryx and Tailwind. Runtime consumption must go through `variables.css`, `astryx.css`, and `tailwind.css` in this package.
